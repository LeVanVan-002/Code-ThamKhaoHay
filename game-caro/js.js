...
function assignRoles() {
    askUser("Bạn có muốn đi trước không?");
...
function restartGame(ask) {
    if (moves > 0) {
        var response = confirm("Bạn có chắc chắn muốn bắt đầu lại không?");
        if (response === false) {
            return;
        }
    }
...
function endGame(who) {
    if (who == player) {
        announceWinner("Chúc mừng bạn đã chiến thắng!");
    } else if (who == computer) {
        announceWinner("Máy tính chiến thắng!| còn non và xanh lắm | biết anh Lê Văn Không ,tao sợ mỗi nó !! ");
    } else {
        announceWinner("Bất phân thắng bại | làm lại nào người ae  !!");
    }
    gameOver = true;
    whoseTurn = 0;
    moves = 0;
